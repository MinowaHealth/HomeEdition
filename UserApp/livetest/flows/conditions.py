"""Live test: /api/v1/conditions CRUD (health_conditions table)."""
...
class ConditionsFlow(Flow):
    name = "conditions"

    def run(self) -> FlowResult:
        cur = self.conn.cursor()
        condition_id: str | None = None
        condition_name = f"livetest-condition-{uuid.uuid4().hex[:8]}"
        before = 0
        ...
        with self.step("POST /api/v1/conditions"):
            resp = self.session.post(
                f"{self.cfg.base_url}/api/v1/conditions",
                json={
                    "name": condition_name,
                    "status": "active",
                    "severity": "mild",
        ...
        with self.step("GET /api/v1/conditions includes created condition"):
            assert condition_id is not None, "condition_id not set"
            ...
            assert any(c.get("id") == condition_id for c in items), (
                f"condition {condition_id} not in GET response "
                f"({len(items)} items returned)"
            )

        with self.step("GET /api/v1/conditions/{id}"):
            assert condition_id is not None, "condition_id not set"
            resp = self.session.get(
                f"{self.cfg.base_url}/api/v1/conditions/{condition_id}",
                timeout=self.cfg.timeout,
            )
            assert resp.status_code == 200, (
                f"expected 200, got {resp.status_code}: {resp.text}"
            ...
            body = resp.json()
            assert body.get("name") == condition_name, (
                f"condition name mismatch: got {body.get('name')!r}, "
                f"expected {condition_name!r}"
            )

        with self.step("PUT /api/v1/conditions/{id}"):
            assert condition_id is not None, "condition_id not set"
        ...
        with self.step("DELETE /api/v1/conditions/{id}"):
            assert condition_id is not None, "condition_id not set"
...
def main() -> None:
    ...
    try:
        flow = ConditionsFlow(cfg, session, conn, user_id)
        result = flow.run()
    finally:
        conn.close()
